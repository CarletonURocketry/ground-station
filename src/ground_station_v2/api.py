...
@app.get("/missions")
async def get_missions(client_id: str, source: str = "recordings"):
    import os
    ...
    if source == "missions":
        ...
        if os.path.exists(missions_dir):
            for entry in os.listdir(missions_dir):
                entry_path = os.path.join(missions_dir, entry)
                if os.path.isfile(entry_path) and entry.endswith(".mission"):
                    # Remove .mission extension from name
                    name = entry[:-8]  # Remove last 8 characters (.mission)
                    missions.append({
                        "name": name,
                        "path": entry_path
                    })
        ...
        if os.path.exists(recordings_dir):
            for entry in os.listdir(recordings_dir):
                ...
                if os.path.isdir(entry_path):
                    raw_file = os.path.join(entry_path, "raw")
    ...
    missions.sort(key=lambda x: x["name"], reverse=True)
...
async def replay_stop(client_id: str):
    if client_id not in connected_clients:
        raise HTTPException(status_code=401, detail="Client not connected")
...
async def record_start(client_id: str):
    recorder.start()
...
async def record_stop(client_id: str):
    recorder.stop()
