...
def parse_packet_header(header_bytes: bytes) -> PacketHeader:
    ...
    try:
        ...
        timestamp, num_block_headers, packet_num = struct.unpack("<HBB", header_bytes[CALLSIGN_LENGTH:])
        return PacketHeader(callsign, timestamp, num_block_headers, packet_num)
...
def parse_block_header(header_bytes: bytes) -> BlockHeader:
    ...
    try:
        logger.debug(f"{header_bytes=}")
        (type, count) = struct.unpack("<BB", header_bytes)
        return BlockHeader(BlockType(type), count)
